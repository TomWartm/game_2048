...
class DQNetworkLarge(nn.Module):
    def __init__(self):
        super(DQNetworkLarge, self).__init__()
...
class QLearningLarge:
    def __init__(self, model_path="data/qmodel_large.pt"):
        self.counter = 0
        self.training_steps = 0
        self.start_time = datetime.now()
        self.model_path = model_path
        
        self.model = DQNetworkLarge()
    ...
    def load_model(self, from_training_step = 470813):
    ...
    def move(self, game, board):
        ...
        left_bottom_mask = np.array([[1.1, 0.9, 0.9, 0.8],
                                   [1.2, 1.0, 1.0, 0.9],
                                   [1.4, 1.3, 1.1, 1.1],
                                   [1.5, 1.4, 1.3, 1.2]])
        reward *= np.sum(np.multiply(next_board, left_bottom_mask)) / np.max(next_board)
